...
function MaskModal({ mask, onSave, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:999,
      ...
      <div style={{background:"#131a2e",border:"1px solid rgba(255,255,255,0.12)",
        ...
        <p style={{color:"#64748b",fontSize:"11px",margin:"0 0 8px",fontFamily:"'IBM Plex Mono',monospace",
          textTransform:"uppercase",letterSpacing:"0.05em"}}>CAMPOS</p>
        ...
        <button onClick={addField} style={{background:"rgba(255,255,255,0.05)",
          ...
          fontSize:"13px",fontFamily:"'IBM Plex Mono',monospace"}}>+ Adicionar Campo</button>

        <p style={{color:"#64748b",fontSize:"11px",margin:"0 0 8px",fontFamily:"'IBM Plex Mono',monospace",
          textTransform:"uppercase",letterSpacing:"0.05em"}}>TEMPLATE — use {"{Nome do Campo}"}</p>
        ...
        <div style={{display:"flex",gap:"10px",justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{background:"transparent",
            ...
            borderRadius:"8px",padding:"10px 18px",cursor:"pointer",fontSize:"13px"}}>Cancelar</button>
          <button onClick={()=>onSave({...mask,name,emoji,color,fields,template})}
            style={{background:color,border:"none",color:"#000",borderRadius:"8px",
              padding:"10px 22px",cursor:"pointer",fontSize:"14px",fontWeight:"700",
              fontFamily:"'Space Grotesk',sans-serif"}}>Salvar</button>
        </div>
      </div>
    </div>
  );
      ...
      {history.map((item, idx) => {
        ...
        return (
          <div key={item.id} style={{
            ...
            <pre style={{fontSize:"12px",lineHeight:"1.7",color:"#4a5568",
              fontFamily:"'IBM Plex Mono',monospace",whiteSpace:"pre-wrap",
              wordBreak:"break-word",margin:0,maxHeight:"110px",overflowY:"auto"}}>{item.text}</pre>
          </div>
        );
      })}
    </div>
...
export default function App() {
  const [masks,     setMasks]     = useState(() => ls.get(SK_MASKS) || INITIAL_MASKS);
  const [activeId,  setActiveId]  = useState(() => {
    const saved = ls.get(SK_ACTIVE);
    const all   = ls.get(SK_MASKS) || INITIAL_MASKS;
    return all.find(m => m.id === saved) ? saved : all[0].id;
  });
  ...
  useEffect(() => ls.set(SK_MASKS,   masks),   [masks]);
  useEffect(() => ls.set(SK_ACTIVE,  activeId),[activeId]);
  useEffect(() => ls.set(SK_HISTORY, history), [history]);

  const activeMask = masks.find(m => m.id === activeId) || masks[0];
  const output     = buildOutput(activeMask, values);
  ...
  const handleSaveMask = mask => {
    if (!mask.id) {
      const nm = { ...mask, id: Date.now() };
      setMasks(p => [...p, nm]);
      setActiveId(nm.id);
    } else {
      setMasks(p => p.map(m => m.id === mask.id ? mask : m));
    }
    setModalMask(null); setValues({});
  ...
  const handleDeleteMask = id => {
    ...
    setMasks(next); setActiveId(next[0].id); setValues({});
  };
  ...
  const Badge = ({ count, color }) => count > 0 ? (
    <span style={{background:color,color:"#000",borderRadius:"10px",
      padding:"0 6px",fontSize:"10px",fontWeight:"700",lineHeight:"18px"}}>{count}</span>
  ) : null;
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:"#0a0f1e",
        ...
        {/* ── Header ── */}
        <div className="app-header" style={{borderBottom:"1px solid rgba(255,255,255,0.08)",
        ...
        {/* ── Mask tab bar ── */}
        <div className="mask-tabs" style={{display:"flex",gap:"5px",padding:"11px 22px 0",
          ...
          <button onClick={()=>setModalMask({name:"",emoji:"📄",color:"#00c2ff",fields:[],template:""})}
            style={{padding:"7px 12px",borderRadius:"10px 10px 0 0",cursor:"pointer",
        ...
        <div className="main-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",flex:1,minHeight:0}}>
          ...
          <div style={{
            ...
            <div id="fields-inner" style={{
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"16px"}}>
                ...
                <div style={{display:"flex",gap:"5px"}}>
                  <button onClick={()=>setModalMask(activeMask)} title="Editar" style={{
                    ...
                    color:"#94a3b8",borderRadius:"7px",padding:"4px 8px",cursor:"pointer",fontSize:"12px"}}>✏️</button>
                  {masks.length>1 && (
                    <button onClick={()=>handleDeleteMask(activeMask.id)} title="Excluir" style={{
                      background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",
                      color:"#ef4444",borderRadius:"7px",padding:"4px 8px",cursor:"pointer",fontSize:"12px"}}>🗑️</button>
                  )}
                </div>
              </div>
              ...
              <button onClick={()=>setValues({})} style={{marginTop:"4px",width:"100%",
              ...
              <div style={{flex:1,padding:"20px",display:"flex",flexDirection:"column",
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}>
                  <p style={{fontSize:"10px",fontWeight:"600",color:"#64748b",
                    fontFamily:"'IBM Plex Mono',monospace",textTransform:"uppercase",letterSpacing:"0.05em"}}>Preview do Relato</p>
                ...
                <pre style={{flex:1,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",
                  ...
                  overflowY:"auto",minHeight:"140px"}}>{output}</pre>
                <button onClick={handleCopy} style={{marginTop:"14px",padding:"13px",borderRadius:"10px",
          ...
          {mobileTab==="preview" && (
            <div style={{padding:"16px",display:"flex",flexDirection:"column",
              ...
              <pre style={{flex:1,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",
                ...
                fontFamily:"'IBM Plex Mono',monospace",whiteSpace:"pre-wrap",wordBreak:"break-word",
                overflowY:"auto",minHeight:"160px",marginBottom:"14px"}}>{output}</pre>
              <button onClick={handleCopy} style={{padding:"13px",borderRadius:"10px",border:"none",
          ...
          {mobileTab==="history" && (
            <div style={{background:"#0e1525",display:"flex",flexDirection:"column",
